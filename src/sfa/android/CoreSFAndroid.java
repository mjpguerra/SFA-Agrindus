            ...
            if (file.isDirectory()) {   
                if(file.list().length > 0){  
                    ...
                    if (ftp.storeFile(arquivoFTP, arqEnviar)) {  
                        ...
                        ftp.disconnect();  
                        if (file2.delete()) {  
                            Log.i(TAG, "Arquivo " + arquivoFTP + " excluído com sucesso");  
                           return true;  
                       } else {  
                        ...
                        ftp.logout();  
                        ftp.disconnect();  
                        Log.i(TAG, "ERRO: arquivo " + arquivoFTP + "não foi enviado!");  
                        return false;  
                    }   
                }  
                else  
                {  
                    Log.i(TAG, "Não existe o arquivo " + arquivoFTP + "neste diretório!");  
                    return false;  
                }   
            }  
            else  
            {  
                Log.i(TAG, "Não é diretório");  
                return false;  
            }  
        } catch (Exception e) {  
