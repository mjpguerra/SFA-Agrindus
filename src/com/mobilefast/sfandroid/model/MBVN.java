package com.mobilefast.sfandroid.model;

import java.io.Serializable;
import java.sql.Timestamp;

public class MBVN implements Serializable {

   private static final long serialVersionUID = 1L;
   private String assina_ped;
   private String cliente_fim;
   private String cliente_ini;
   private Timestamp data_alter;
   private short dec_livre;
   private String em_processo;
   private short empresa;
   private int filial;
   private String filial_alfa;
   private short hora_alter;
   private int id;
   private String intr;
   private short log_num;
   private short log_stat;
   private String mens1;
   private String mens2;
   private String nome_conh;
   private String operador;
   private String pedido_fim;
   private String pedido_ini;
   private double porc_comissao;
   private String proximo_cli;
   private String proximo_ped;
   private int reservado1;
   private Timestamp reservado10;
   private Timestamp reservado11;
   private Timestamp reservado12;
   private String reservado13;
   private String reservado14;
   private String reservado15;
   private String reservado16;
   private int reservado2;
   private int reservado3;
   private int reservado4;
   private double reservado5;
   private double reservado6;
   private double reservado7;
   private double reservado8;
   private Timestamp reservado9;
   private double saldo;
   private String senha;
   private short status;
   private short tam_livre;
   private short time_stamp;
   private String tipo_livre;
   private String titulo_livre;
   private String trata_limite;
   private String vendedor;
   private int version;
   private String D_E_L_E_T_ = null;
   private String D_I_R_T_Y_ = null;

   public MBVN() {
      super();
   }
   public String getD_E_L_E_T_() {
	      return this.D_E_L_E_T_;
	   }

	   public String getD_I_R_T_Y_() {
	      return this.D_I_R_T_Y_;
	   }
	   public void setD_E_L_E_T_(String var1) {
		      this.D_E_L_E_T_ = var1;
		   }

		   public void setD_I_R_T_Y_(String var1) {
		      this.D_I_R_T_Y_ = var1;
		   }
   public String getAssina_ped() {
      return this.assina_ped;
   }

   public String getCliente_fim() {
      return this.cliente_fim;
   }

   public String getCliente_ini() {
      return this.cliente_ini;
   }

   public Timestamp getData_alter() {
      return this.data_alter;
   }

   public short getDec_livre() {
      return this.dec_livre;
   }

   public String getEm_processo() {
      return this.em_processo;
   }

   public short getEmpresa() {
      return this.empresa;
   }

   public int getFilial() {
      return this.filial;
   }

   public String getFilial_alfa() {
      return this.filial_alfa;
   }

   public short getHora_alter() {
      return this.hora_alter;
   }

   public int getId() {
      return this.id;
   }

   public String getIntr() {
      return this.intr;
   }

   public short getLog_num() {
      return this.log_num;
   }

   public short getLog_stat() {
      return this.log_stat;
   }

   public String getMens1() {
      return this.mens1;
   }

   public String getMens2() {
      return this.mens2;
   }

   public String getNome_conh() {
      return this.nome_conh;
   }

   public String getOperador() {
      return this.operador;
   }

   public String getPedido_fim() {
      return this.pedido_fim;
   }

   public String getPedido_ini() {
      return this.pedido_ini;
   }

   public double getPorc_comissao() {
      return this.porc_comissao;
   }

   public String getProximo_cli() {
      return this.proximo_cli;
   }

   public String getProximo_ped() {
      return this.proximo_ped;
   }

   public int getReservado1() {
      return this.reservado1;
   }

   public Timestamp getReservado10() {
      return this.reservado10;
   }

   public Timestamp getReservado11() {
      return this.reservado11;
   }

   public Timestamp getReservado12() {
      return this.reservado12;
   }

   public String getReservado13() {
      return this.reservado13;
   }

   public String getReservado14() {
      return this.reservado14;
   }

   public String getReservado15() {
      return this.reservado15;
   }

   public String getReservado16() {
      return this.reservado16;
   }

   public int getReservado2() {
      return this.reservado2;
   }

   public int getReservado3() {
      return this.reservado3;
   }

   public int getReservado4() {
      return this.reservado4;
   }

   public double getReservado5() {
      return this.reservado5;
   }

   public double getReservado6() {
      return this.reservado6;
   }

   public double getReservado7() {
      return this.reservado7;
   }

   public double getReservado8() {
      return this.reservado8;
   }

   public Timestamp getReservado9() {
      return this.reservado9;
   }

   public double getSaldo() {
      return this.saldo;
   }

   public String getSenha() {
      return this.senha;
   }

   public short getStatus() {
      return this.status;
   }

   public short getTam_livre() {
      return this.tam_livre;
   }

   public short getTime_stamp() {
      return this.time_stamp;
   }

   public String getTipo_livre() {
      return this.tipo_livre;
   }

   public String getTitulo_livre() {
      return this.titulo_livre;
   }

   public String getTrata_limite() {
      return this.trata_limite;
   }

   public String getVendedor() {
      return this.vendedor;
   }

   public int getVersion() {
      return this.version;
   }

   public void setAssina_ped(String var1) {
      this.assina_ped = var1;
   }

   public void setCliente_fim(String var1) {
      this.cliente_fim = var1;
   }

   public void setCliente_ini(String var1) {
      this.cliente_ini = var1;
   }

   public void setData_alter(Timestamp var1) {
      this.data_alter = var1;
   }

   public void setDec_livre(short var1) {
      this.dec_livre = var1;
   }

   public void setEm_processo(String var1) {
      this.em_processo = var1;
   }

   public void setEmpresa(short var1) {
      this.empresa = var1;
   }

   public void setFilial(int var1) {
      this.filial = var1;
   }

   public void setFilial_alfa(String var1) {
      this.filial_alfa = var1;
   }

   public void setHora_alter(short var1) {
      this.hora_alter = var1;
   }

   public void setId(int var1) {
      this.id = var1;
   }

   public void setIntr(String var1) {
      this.intr = var1;
   }

   public void setLog_num(short var1) {
      this.log_num = var1;
   }

   public void setLog_stat(short var1) {
      this.log_stat = var1;
   }

   public void setMens1(String var1) {
      this.mens1 = var1;
   }

   public void setMens2(String var1) {
      this.mens2 = var1;
   }

   public void setNome_conh(String var1) {
      this.nome_conh = var1;
   }

   public void setOperador(String var1) {
      this.operador = var1;
   }

   public void setPedido_fim(String var1) {
      this.pedido_fim = var1;
   }

   public void setPedido_ini(String var1) {
      this.pedido_ini = var1;
   }

   public void setPorc_comissao(double var1) {
      this.porc_comissao = var1;
   }

   public void setProximo_cli(String var1) {
      this.proximo_cli = var1;
   }

   public void setProximo_ped(String var1) {
      this.proximo_ped = var1;
   }

   public void setReservado1(int var1) {
      this.reservado1 = var1;
   }

   public void setReservado10(Timestamp var1) {
      this.reservado10 = var1;
   }

   public void setReservado11(Timestamp var1) {
      this.reservado11 = var1;
   }

   public void setReservado12(Timestamp var1) {
      this.reservado12 = var1;
   }

   public void setReservado13(String var1) {
      this.reservado13 = var1;
   }

   public void setReservado14(String var1) {
      this.reservado14 = var1;
   }

   public void setReservado15(String var1) {
      this.reservado15 = var1;
   }

   public void setReservado16(String var1) {
      this.reservado16 = var1;
   }

   public void setReservado2(int var1) {
      this.reservado2 = var1;
   }

   public void setReservado3(int var1) {
      this.reservado3 = var1;
   }

   public void setReservado4(int var1) {
      this.reservado4 = var1;
   }

   public void setReservado5(double var1) {
      this.reservado5 = var1;
   }

   public void setReservado6(double var1) {
      this.reservado6 = var1;
   }

   public void setReservado7(double var1) {
      this.reservado7 = var1;
   }

   public void setReservado8(double var1) {
      this.reservado8 = var1;
   }

   public void setReservado9(Timestamp var1) {
      this.reservado9 = var1;
   }

   public void setSaldo(double var1) {
      this.saldo = var1;
   }

   public void setSenha(String var1) {
      this.senha = var1;
   }

   public void setStatus(short var1) {
      this.status = var1;
   }

   public void setTam_livre(short var1) {
      this.tam_livre = var1;
   }

   public void setTime_stamp(short var1) {
      this.time_stamp = var1;
   }

   public void setTipo_livre(String var1) {
      this.tipo_livre = var1;
   }

   public void setTitulo_livre(String var1) {
      this.titulo_livre = var1;
   }

   public void setTrata_limite(String var1) {
      this.trata_limite = var1;
   }

   public void setVendedor(String var1) {
      this.vendedor = var1;
   }

   public void setVersion(int var1) {
      this.version = var1;
   }
}
