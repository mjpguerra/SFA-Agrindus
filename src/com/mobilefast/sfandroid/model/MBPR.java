package com.mobilefast.sfandroid.model;

import java.io.Serializable;
import java.sql.Timestamp;

public class MBPR implements Serializable {

   private static final long serialVersionUID = 1L;
   private String c_prod;
   private String c_prod_palm;
   private String cod_barra;
   private Timestamp data_alter;
   private short deposito;
   private String descricao;
   private short empresa;
   private short estab;
   private double fator_uni;
   private short filial;
   private String grupo;
   private short hora_alter;
   private int id;
   private String intr;
   private String local_padrao;
   private String operador;
   private double peso_bruto;
   private double porc_comissao;
   private double porc_icm;
   private double porc_ipi;
   private String posicao_estq;
   private int prazo_entrega;
   private double preco_venda;
   private double qtde_p_embal;
   private int reservado1;
   private double reservado10;
   private double reservado11;
   private double reservado12;
   private double reservado13;
   private double reservado14;
   private double reservado15;
   private double reservado16;
   private int reservado2;
   private double reservado3;
   private double reservado4;
   private Timestamp reservado5;
   private Timestamp reservado6;
   private String reservado7;
   private String reservado8;
   private double reservado9;
   private short time_stamp;
   private String tipo_conv;
   private String unid_alt;
   private String unidade;
   private String vendedor;
   private int version;


   public MBPR() {
      super();
   }

   public String getC_prod() {
      return this.c_prod;
   }

   public String getC_prod_palm() {
      return this.c_prod_palm;
   }

   public String getCod_barra() {
      return this.cod_barra;
   }

   public Timestamp getData_alter() {
      return this.data_alter;
   }

   public short getDeposito() {
      return this.deposito;
   }

   public String getDescricao() {
      return this.descricao;
   }

   public short getEmpresa() {
      return this.empresa;
   }

   public short getEstab() {
      return this.estab;
   }

   public double getFator_uni() {
      return this.fator_uni;
   }

   public short getFilial() {
      return this.filial;
   }

   public String getGrupo() {
      return this.grupo;
   }

   public short getHora_alter() {
      return this.hora_alter;
   }

   public int getId() {
      return this.id;
   }

   public String getIntr() {
      return this.intr;
   }

   public String getLocal_padrao() {
      return this.local_padrao;
   }

   public String getOperador() {
      return this.operador;
   }

   public double getPeso_bruto() {
      return this.peso_bruto;
   }

   public double getPorc_comissao() {
      return this.porc_comissao;
   }

   public double getPorc_icm() {
      return this.porc_icm;
   }

   public double getPorc_ipi() {
      return this.porc_ipi;
   }

   public String getPosicao_estq() {
      return this.posicao_estq;
   }

   public int getPrazo_entrega() {
      return this.prazo_entrega;
   }

   public double getPreco_venda() {
      return this.preco_venda;
   }

   public double getQtde_p_embal() {
      return this.qtde_p_embal;
   }

   public int getReservado1() {
      return this.reservado1;
   }

   public double getReservado10() {
      return this.reservado10;
   }

   public double getReservado11() {
      return this.reservado11;
   }

   public double getReservado12() {
      return this.reservado12;
   }

   public double getReservado13() {
      return this.reservado13;
   }

   public double getReservado14() {
      return this.reservado14;
   }

   public double getReservado15() {
      return this.reservado15;
   }

   public double getReservado16() {
      return this.reservado16;
   }

   public int getReservado2() {
      return this.reservado2;
   }

   public double getReservado3() {
      return this.reservado3;
   }

   public double getReservado4() {
      return this.reservado4;
   }

   public Timestamp getReservado5() {
      return this.reservado5;
   }

   public Timestamp getReservado6() {
      return this.reservado6;
   }

   public String getReservado7() {
      return this.reservado7;
   }

   public String getReservado8() {
      return this.reservado8;
   }

   public double getReservado9() {
      return this.reservado9;
   }

   public short getTime_stamp() {
      return this.time_stamp;
   }

   public String getTipo_conv() {
      return this.tipo_conv;
   }

   public String getUnid_alt() {
      return this.unid_alt;
   }

   public String getUnidade() {
      return this.unidade;
   }

   public String getVendedor() {
      return this.vendedor;
   }

   public int getVersion() {
      return this.version;
   }

   public void setC_prod(String var1) {
      this.c_prod = var1;
   }

   public void setC_prod_palm(String var1) {
      this.c_prod_palm = var1;
   }

   public void setCod_barra(String var1) {
      this.cod_barra = var1;
   }

   public void setData_alter(Timestamp var1) {
      this.data_alter = var1;
   }

   public void setDeposito(short var1) {
      this.deposito = var1;
   }

   public void setDescricao(String var1) {
      this.descricao = var1;
   }

   public void setEmpresa(short var1) {
      this.empresa = var1;
   }

   public void setEstab(short var1) {
      this.estab = var1;
   }

   public void setFator_uni(double var1) {
      this.fator_uni = var1;
   }

   public void setFilial(short var1) {
      this.filial = var1;
   }

   public void setGrupo(String var1) {
      this.grupo = var1;
   }

   public void setHora_alter(short var1) {
      this.hora_alter = var1;
   }

   public void setId(int var1) {
      this.id = var1;
   }

   public void setIntr(String var1) {
      this.intr = var1;
   }

   public void setLocal_padrao(String var1) {
      this.local_padrao = var1;
   }

   public void setOperador(String var1) {
      this.operador = var1;
   }

   public void setPeso_bruto(double var1) {
      this.peso_bruto = var1;
   }

   public void setPorc_comissao(double var1) {
      this.porc_comissao = var1;
   }

   public void setPorc_icm(double var1) {
      this.porc_icm = var1;
   }

   public void setPorc_ipi(double var1) {
      this.porc_ipi = var1;
   }

   public void setPosicao_estq(String var1) {
      this.posicao_estq = var1;
   }

   public void setPrazo_entrega(int var1) {
      this.prazo_entrega = var1;
   }

   public void setPreco_venda(double var1) {
      this.preco_venda = var1;
   }

   public void setQtde_p_embal(double var1) {
      this.qtde_p_embal = var1;
   }

   public void setReservado1(int var1) {
      this.reservado1 = var1;
   }

   public void setReservado10(double var1) {
      this.reservado10 = var1;
   }

   public void setReservado11(double var1) {
      this.reservado11 = var1;
   }

   public void setReservado12(double var1) {
      this.reservado12 = var1;
   }

   public void setReservado13(double var1) {
      this.reservado13 = var1;
   }

   public void setReservado14(double var1) {
      this.reservado14 = var1;
   }

   public void setReservado15(double var1) {
      this.reservado15 = var1;
   }

   public void setReservado16(double var1) {
      this.reservado16 = var1;
   }

   public void setReservado2(int var1) {
      this.reservado2 = var1;
   }

   public void setReservado3(double var1) {
      this.reservado3 = var1;
   }

   public void setReservado4(double var1) {
      this.reservado4 = var1;
   }

   public void setReservado5(Timestamp var1) {
      this.reservado5 = var1;
   }

   public void setReservado6(Timestamp var1) {
      this.reservado6 = var1;
   }

   public void setReservado7(String var1) {
      this.reservado7 = var1;
   }

   public void setReservado8(String var1) {
      this.reservado8 = var1;
   }

   public void setReservado9(double var1) {
      this.reservado9 = var1;
   }

   public void setTime_stamp(short var1) {
      this.time_stamp = var1;
   }

   public void setTipo_conv(String var1) {
      this.tipo_conv = var1;
   }

   public void setUnid_alt(String var1) {
      this.unid_alt = var1;
   }

   public void setUnidade(String var1) {
      this.unidade = var1;
   }

   public void setVendedor(String var1) {
      this.vendedor = var1;
   }

   public void setVersion(int var1) {
      this.version = var1;
   }

   public String toString() {
      StringBuffer var1 = new StringBuffer();
      var1.append("com.akcess.vo.Mbpr :");
      var1.append("empresa=\'" + this.empresa + "\'");
      var1.append(", filial=\'" + this.filial + "\'");
      var1.append(", vendedor=\'" + this.vendedor + "\'");
      var1.append(", estab=\'" + this.estab + "\'");
      var1.append(", deposito=\'" + this.deposito + "\'");
      var1.append(", c_prod=\'" + this.c_prod + "\'");
      var1.append(", c_prod_palm=\'" + this.c_prod_palm + "\'");
      var1.append(", descricao=\'" + this.descricao + "\'");
      var1.append(", unidade=\'" + this.unidade + "\'");
      var1.append(", porc_icm=\'" + this.porc_icm + "\'");
      var1.append(", porc_ipi=\'" + this.porc_ipi + "\'");
      var1.append(", peso_bruto=\'" + this.peso_bruto + "\'");
      var1.append(", qtde_p_embal=\'" + this.qtde_p_embal + "\'");
      var1.append(", prazo_entrega=\'" + this.prazo_entrega + "\'");
      var1.append(", porc_comissao=\'" + this.porc_comissao + "\'");
      var1.append(", cod_barra=\'" + this.cod_barra + "\'");
      var1.append(", local_padrao=\'" + this.local_padrao + "\'");
      var1.append(", posicao_estq=\'" + this.posicao_estq + "\'");
      var1.append(", unid_alt=\'" + this.unid_alt + "\'");
      var1.append(", fator_uni=\'" + this.fator_uni + "\'");
      var1.append(", tipo_conv=\'" + this.tipo_conv + "\'");
      var1.append(", preco_venda=\'" + this.preco_venda + "\'");
      var1.append(", reservado1=\'" + this.reservado1 + "\'");
      var1.append(", reservado2=\'" + this.reservado2 + "\'");
      var1.append(", reservado3=\'" + this.reservado3 + "\'");
      var1.append(", reservado4=\'" + this.reservado4 + "\'");
      var1.append(", reservado5=\'" + this.reservado5 + "\'");
      var1.append(", reservado6=\'" + this.reservado6 + "\'");
      var1.append(", reservado7=\'" + this.reservado7 + "\'");
      var1.append(", reservado8=\'" + this.reservado8 + "\'");
      var1.append(", reservado9=\'" + this.reservado9 + "\'");
      var1.append(", reservado10=\'" + this.reservado10 + "\'");
      var1.append(", intr=\'" + this.intr + "\'");
      var1.append(", operador=\'" + this.operador + "\'");
      var1.append(", data_alter=\'" + this.data_alter + "\'");
      var1.append(", hora_alter=\'" + this.hora_alter + "\'");
      var1.append(", time_stamp=\'" + this.time_stamp + "\'");
      var1.append(", reservado11=\'" + this.reservado11 + "\'");
      var1.append(", reservado12=\'" + this.reservado12 + "\'");
      var1.append(", reservado13=\'" + this.reservado13 + "\'");
      var1.append(", reservado14=\'" + this.reservado14 + "\'");
      var1.append(", reservado15=\'" + this.reservado15 + "\'");
      var1.append(", reservado16=\'" + this.reservado16 + "\'");
      var1.append(", version=\'" + this.version + "\'");
      var1.append(", grupo=\'" + this.grupo + "\'");
      return var1.toString();
   }
}
